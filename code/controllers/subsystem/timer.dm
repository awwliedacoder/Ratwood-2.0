 ...
 *
 * Additional instances of this subsystem can be created via TIMER_SUBSYSTEM_DEF; timers are routed
 * to them with the timer_subsystem argument of addtimer/deltimer/timeleft/updatetimedelay.
 */
SUBSYSTEM_DEF(timer)
	name = "Timer"
...
/datum/controller/subsystem/timer/Recover()
	...
	for(var/global_var in global.vars)
		var/datum/controller/subsystem/timer/candidate = global.vars[global_var]
		if (istype(candidate, src.type) && candidate.type == src.type)
			timerSS = candidate
...
/datum/timedevent/New(datum/callback/callBack, wait, flags, datum/controller/subsystem/timer/timer_subsystem, hash, source)
	...
	if (flags & TIMER_UNIQUE)
		src.timer_subsystem.hashes[hash] = src
	...
	if (flags & TIMER_STOPPABLE)
		...
		if (nextid >= SHORT_REAL_LIMIT)
			...
			nextid++
		src.timer_subsystem.timer_id_dict[id] = src

	if ((timeToRun < world.time || timeToRun < src.timer_subsystem.head_offset) && !(flags & TIMER_CLIENT_TIME))
		CRASH("Invalid timer state: Timer created that would require a backtrack to run (addtimer would never let this happen): [src.timer_subsystem.get_timer_debug_string(src)]")
...
/datum/timedevent/proc/bucketEject()
	...
	// the head of the bucket, or has otherwise joined a bucket, or is
	// otherwise not present in the secondary queue
	if(buckethead == src)
...
/datum/timedevent/proc/bucketJoin()
	if (bucket_joined)
		stack_trace("Bucket already joined! [get_name()]")
	...
	if (bucket_pos < timer_subsystem.practical_offset && timeToRun < (timer_subsystem.head_offset + TICKS2DS(BUCKET_LEN)))
		WARNING("Bucket pos in past: bucket_pos = [bucket_pos] < practical_offset = [timer_subsystem.practical_offset] \
			&& timeToRun = [timeToRun] < [timer_subsystem.head_offset + TICKS2DS(BUCKET_LEN)], Timer: [get_name()]")
...
/datum/timedevent/proc/get_name()
	if (!callBack)
		return "Timer: [id] (\ref[src]), TTR: [timeToRun], source: [source], NO CALLBACK"
	return "Timer: [id] (\ref[src]), TTR: [timeToRun], Flags: [jointext(bitfield2list(flags, list("TIMER_UNIQUE", "TIMER_OVERRIDE", "TIMER_CLIENT_TIME", "TIMER_STOPPABLE", "TIMER_NO_HASH_WAIT", "TIMER_LOOP", "TIMER_DELETE_ME")), ", ")], callBack: \ref[callBack], callBack.object: [callBack.object]\ref[callBack.object]([getcallingtype()]), callBack.delegate:[callBack.delegate]([callBack.arguments ? callBack.arguments.Join(", ") : ""]), source: [source]"
 ...
 * * timer_subsystem the timer subsystem to insert this timer into, defaults to SStimer
 */
/proc/_addtimer(datum/callback/callback, wait = 0, flags = 0, datum/controller/subsystem/timer/timer_subsystem, file, line)
	if (!istype(callback))
		CRASH("addtimer called [callback ? "with an invalid callback ([callback])" : "without a callback"]")

	if (!isnum(wait))
		CRASH("addtimer called with a non-numeric wait ([wait])")
 ...
 * * timer_subsystem the timer subsystem the timer lives in, defaults to SStimer
 */
