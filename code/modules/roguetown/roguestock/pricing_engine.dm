...
/proc/init_material_baseline_prices()
	...
	// Ratwood-only material. Everything else that shows in the missing-materials audit is
	// either shared with AP's recipes and equally unpriced there (bone, brick, whetstone,
	// ash, the alch dusts, ...) - matching upstream's live pricing behavior is worth more
	// than patching their holes unilaterally - or loot-economy items (decrepit ingots,
	// magic drops) where the all-unpriced fallback correctly keeps hand-tuned sellprices.
